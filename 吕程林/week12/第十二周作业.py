...
DBRX	RoPE	串行	grouped query	gated形式	LayerNorm	没有ffn_act_fn这个参数	无bias	
deepseek	RoPE	串行	grouped query	gated形式	RMSnorm/pre norm	SiLU	无bias	
gemma	RoPE	串行	grouped query	gated形式	RMSnorm/pre norm	gelu	无bias	
grok1	RoPE	串行	grouped query	gated形式	RMSnorm/pre norm	GELU	无bias	
