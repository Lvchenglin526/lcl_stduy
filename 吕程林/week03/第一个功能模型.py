...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        # lx = lx.squeeze()                            #(batch_size, vector_dim, 1) -> (batch_size, vector_dim)
        output, lx = self.layer(x)   # output.shape=样本数*文本长度*输出维度 lx.shape = 1*样本数*输出维度
        # print(output.shape)
        # print(lx.shape)
        ...
        # print(y_pred)
        # print(y_pred.shape)
        # print(y.shape)         #(batch_size, 1) -> (batch_size, 1)
        if y is not None:
...
def evaluate(model, vocab, sample_length):
    ...
    # print("本次预测集中共有%d个正样本，%d个负样本"%(Y1, Y2))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)      #模型预测
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        print("输入：%s, 预测类别：%d类" % (input_string, np.argmax(result[i]))) # 打印结果



if __name__ == "__main__":
    main()
    test_strings = ["fnvf我e", "wz你dfg", "rqwdeg", "n我kwww", "rqadeg"]
