...
class DataGenerator:
    ...
    def load(self):
        ...
        with open(self.path, encoding="utf8") as f:
            for line in f:
                line = json.loads(line)  # 按行进行扫描
                #加载训练集
                if isinstance(line, dict):  # 将一行中的questions和target分开
                    self.data_type = "train"
                    questions = line["questions"]
                    label = line["target"]
                    for question in questions:  # 因为是列表的格式，所以可以迭代
                        input_id = self.encode_sentence(question)  # 每个问题进行向量化
                        input_id = torch.LongTensor(input_id)  # 向量化后的结果保存为张量
                        self.knwb[self.schema[label]].append(input_id)  # 对应向量结果保存到对应的数字中
                #加载测试集
                else:
                    self.data_type = "test"
    ...
    def random_train_sample(self):
        standard_question_index = list(self.knwb.keys())  # self.knwb.keys()是数字
        # 随机样本
        ...
        if len(self.knwb[p]) < 2:
            ...
            return [s1, s2, s3, torch.LongTensor([p])]
        # 随机负样本
        # else:
        #     p, n = random.sample(standard_question_index, 2)
